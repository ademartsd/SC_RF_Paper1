...
presets = {
    "SE deep":  {"VS_MIN": 0.40, "VS_MAX": 4.7, "DEPTH_MIN": 0.0, "DEPTH_MAX": 55.0},
    }
...
threshold_vs = 4000 # in case you want to discard the ones where vs[-1]< 4000
...
for i in range(len(df_postburn)):
    ...
    # Sediment properties
    vs0 = vs[0]
    vpvs0 = float(df_postburn.iloc[i]["VpVs0"])
    ...
    vs0_values.append(vs0)
    vpvs0_values.append(vpvs0)
...
vs0_values = np.array(vs0_values)
vpvs0_values = np.array(vpvs0_values)
...
vs0_mean_km = np.mean(vs0_values) / 1000
vs0_std_km = np.std(vs0_values, ddof=1) / 1000

vpvs0_mean = np.mean(vpvs0_values)
vpvs0_std = np.std(vpvs0_values, ddof=1)
...
summary_text = (
    ...
    f"Estimated Sediment velocity:      {vs0_mean_km:.2f} ± {vs0_std_km:.2f} km/s\n"
    f"Estimated Sediment Vp/Vs0:        {vpvs0_mean:.2f} ± {vpvs0_std:.2f}\n"
...
plt.xlabel('Vs (m s$^{-1}$)')
plt.ylabel('Depth (km)')
#plt.title(f'Top {number_to_plot} Best Models for Station ${station}$')
...
if FAST_VIEW_MODE:
    radials = radials[:1] #it should be 10 !!

#radials = RFStream([tr for tr in radials if len(tr.data) != 820])
...
# ==================================================================
#           INSERT REPRESENTATIVE MODEL CALCULATION HERE
# ==================================================================
